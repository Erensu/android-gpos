/*------------------------------------------------------------------------------
 * sppflt.c: GNSS navigation filter using android raw gnss measurement data
 *
 * author   : sujinglan
 * version  : $Revision: 1.2 $ $Date: 2008/07/14 00:05:05 $
 * history  : 2022/12/17 1.0  new
 *-----------------------------------------------------------------------------*/
#include "agpos.h"
...
#define DETECT_OUTL  0            /* detect measurement outliers */
...
#define ANX(opt)     (ANP(opt)+ANC(opt)+AND(opt)+ANT(opt))
#define AIP(opt)     (0)
#define AIV(opt)     (3)
#define AIC(s,f,opt) (ANP(opt)+(s)*NFREQ+f)
#define AID(opt)     (ANP(opt)+ANC(opt))
#define AIT(opt)     (ANP(opt)+ANC(opt)+AND(opt))
#define AIC_(s,f)    ((s)*NFREQ+(f))

#define AOBS_I(val)  (((val)>>16)&0xFF)
#define AOBS_F(val)  (((val)>> 8)&0xFF)
#define AOBS_T(val)  (((val)>> 4)&0xF)
#define AEXC_I(i,f)  (NFREQ*(i)+(f))
...
static int determine_init_state(const asflt_t *flt, const gtime_t cur_time, const prcopt_t *opt)
{
    ...
    for (var=i=0;i<3;i++) var+=flt->P[i+i*flt->nx];
...
static int chk_init_cond(const aslsq_t *lsq, const prcopt_t *opt, int reset_flag)
{
    ...
    if (sqrt(Qpe[0]+Qpe[4])>min_sig_pos||sqrt((Qpe[0]+Qpe[4])/SQR(lsq->sig0_p0))>(reset_flag?60.0:100.0)) {
...
static void chk_rcv_clk_init(asflt_t *flt, const aslsq_t *lsq)
{
    int i,f;

    for (i=0;i<4;i++) {
        for (f=0;f<NFREQ;f++) {
            if (!lsq->psta||!lsq->dtr[AIC_(i,f)]) continue;
            if (flt->x[AIC(i,f,opt)]) continue;
            initxP(flt->x,flt->P,lsq->dtr[AIC_(i,f)],VAR_CLK,AIC(i,f,opt),flt->nx);
            trace(3,"rcv clock init: i=%d\n",i);
        }
    }
}
...
static int init_flt_state(agpos_t *agp, const aobsd_t *obs, int n, int init_flag,
                          int reset_flg, double *x0, double *P0)
{
    const aslsq_t *lsq=&agp->spp.lsq;
    int i,f,nx=agp->spp.flt.nx;
    double *x,*P;
    ...
    chk_rcv_clk_init(flt,&agp->spp.lsq);
    ...
    for (i=0;i<4;i++) {
        for (f=0;f<NFREQ;f++) {
            initxP(x,P,lsq->dtr[AIC_(i,f)],VAR_CLK,AIC(i,f,opt),nx);
        }
    }
    initxP(x,P,lsq->ddrt,VAR_CLK,AID(opt),nx);
...
static void est_prc_psd(agpos_t *agp, const aobsd_t *obs, int n, const nav_t *nav,
                        const double dt, double *cpsd)
{
    ...
    ecef2enu(pos,acc,ae);
    for (i=0;i<2;i++) cpsd[i]=SQR(ae[i])*dt;
    ptime=obs[0].time;
                      ...
                      double *x, double *P)
{
    ...
    /* estimate process noise PSD */
    est_prc_psd(agp,obs,n,nav,dt,psd);

    trace(3,"psd=%.3lf %.3lf %.3lf\n",psd[0],psd[1],psd[2]);

    /* update receiver position/velocity */
    for (i=0;i<3;i++) {
        x[i+AIP(opt)]=x[i+AIP(opt)]+x[i+AIV(opt)]*dt;
    }
    /* update receiver clock bias */
    for (i=0;i<4;i++) {
        for (f=0;f<NFREQ;f++) {
            if (!x[AIC(i,f,opt)]) continue;
            x[AIC(i,f,opt)]=flt->x[AIC(i,f,opt)]+flt->x[AID(opt)]*dt;
        }
    }
    /* receiver clock bias rate */
    x[AID(opt)]=flt->x[AID(opt)];

    F=eye(nx); FP=mat(nx,nx); Qk=zeros(nx,nx);
    ...
    for (i=0;i<4;i++) {
        for (f=0;f<NFREQ;f++) {
            if (!x[AIC(i,f,opt)]) continue;
            F[AID(opt)+AID(opt)*nx]=exp(-dt/CORR_TIME_CLKD);
            F[AIC(i,f,opt)+AID(opt)*nx]=CORR_TIME_CLKD-CORR_TIME_CLKD*exp(-dt/CORR_TIME_CLKD);
        }
    }
    ...
    ecef2pos(x,pos);
    Q[0]=psd[0]; Q[4]=psd[1]; Q[8]=psd[2];
    covecef(pos,Q,Qv);

    /* system noise matrix for position/velocity */
    for (i=AIP(opt);i<AIP(opt)+3;i++) {
        for (j=AIP(opt);j<AIP(opt)+3;j++) Qk[i+j*nx]=1.0/3.0*Qv[i-AIP(opt)+(j-AIP(opt))*3]*pow(dt,3.0);
        for (j=AIV(opt);j<AIV(opt)+3;j++) Qk[i+j*nx]=1.0/2.0*Qv[i-AIP(opt)+(j-AIV(opt))*3]*SQR(dt);
    }
    for (i=AIV(opt);i<AIV(opt)+3;i++) {
        for (j=AIP(opt);j<AIP(opt)+3;j++) Qk[i+j*nx]=1.0/2.0*Qv[i-AIV(opt)+(j-AIP(opt))*3]*SQR(dt);
        for (j=AIV(opt);j<AIV(opt)+3;j++) Qk[i+j*nx]=Qv[i-AIV(opt)+(j-AIV(opt))*3]*dt;
    }
    /* system noise matrix for rcv clock/clock rate */
    for (i=0;i<4;i++) {
        for (f=0;f<NFREQ;f++) {
            if (!x[AIC(i,f,opt)]) continue;
            Qk[AIC(i,f,opt)+AIC(i,f,opt)*nx]=1.0/3.0*psd[3]*pow(dt,3.0)+psd[4]*dt;
            Qk[AIC(i,f,opt)+AID(opt)*nx]=0.5*psd[3]*SQR(dt);
            Qk[AID(opt)+AIC(i,f,opt)*nx]=0.5*psd[3]*SQR(dt);
            Qk[AID(opt)+AID(opt)*nx]=psd[3]*dt;
        }
    }
...
static int prng_res(const aobsd_t *obs, int n, const nav_t *nav, int uf, int uCN0,
                    int uLSQ, int uSig0, const double *x, agpos_t *agp, double *v, double *H, double *var,
                    const int *exc, int *ind)
{
    double r,dion,dtrp,vmeas,vion,vtrp,rr[3],pos[3],e[3],P,lam_L1,azel[2],sig;
    double sig0=agp->spp.lsq.sig0_p0;
    double sat_pr,corr_dt=0.0,freq;
    ...
    if (CORR_COARSE_TIME) corr_dt=x[AIT(opt)];
    else corr_dt=agp->spp.lsq.dt;
    ...
    for (nv=f=0;f<nf;f++) {
        for (i=0;i<n&&i<MAXOBS;i++) {
            ...
            if ((r=geodist(ssat[sat-1].rs,x,e))<=0.0) continue;
            satazel(pos,e,azel);
            ...
            v[nv]=P-(r-CLIGHT*ssat[sat-1].dts[0]+dion+dtrp);
            if (H) {
                for (k=AIP(opt);k<(AIP(opt)+3);k++) H[k+nx*nv]=-e[k];
            }
            /* receiver clock bias offset correction */
            if      (sys==SYS_GPS) {v[nv]-=x[AIC(0,f,opt)]; if (H) H[AIC(0,f,opt)+nv*nx]=1.0;}
            else if (sys==SYS_GLO) {v[nv]-=x[AIC(1,f,opt)]; if (H) H[AIC(1,f,opt)+nv*nx]=1.0;}
            else if (sys==SYS_GAL) {v[nv]-=x[AIC(2,f,opt)]; if (H) H[AIC(2,f,opt)+nv*nx]=1.0;}
            else if (sys==SYS_CMP) {v[nv]-=x[AIC(3,f,opt)]; if (H) H[AIC(3,f,opt)+nv*nx]=1.0;}
            else if (sys==SYS_QZS) {v[nv]-=x[AIC(0,f,opt)]; if (H) H[AIC(0,f,opt)+nv*nx]=1.0;}
            else continue;
            ...
            v[nv]-=sat_pr*corr_dt;
            if (H) {
                if (CORR_COARSE_TIME) H[AIT(opt)+nv*nx]=sat_pr;
            }
            ...
            if (azel[1]<opt->elmincode) continue;

            /* excluded satellite? */
            if (uLSQ&&!ssat[sat-1].vs_lsq[f]) continue;
            if ((exc&&exc[AEXC_I(i,f)])) continue;
            ...
            trace(3,"%4s sat=%3d azel=%5.1f %5.1f res=%10.3f cdt=%10.3f sig=%8.3f cn0=%8.3lf f=%d stat=%4d\n",
                  ...
                  v[nv-1],sat_pr*corr_dt,sqrt(sig),obs[i].SNR[f]*0.25,f,obs[i].unc.stat[f]);
        }
    }
    return nv;
}
                    ...
                    const int *exc, int *ind)
{
    ...
    for (f=0;f<NFREQ;f++) {
        for (i=0;i<n&&i<MAXOBS;i++) {
            ...
            ssat[obs[i].sat-1].vd_flt[f]=0; sat=obs[i].sat;
            ...
            /* doppler residual */
            v[nv]=y-(rate+x[AID(opt)]-CLIGHT*(dts[1]+dts[2]*corr_dt));
            ...
            trace(3,"%4s sat=%3d azel=%5.1f %5.1f res=%8.3f prr=%8.3lf sig=%8.3lf cn0=%8.3lf f=%d\n",
                  prn,obs[i].sat,azel[0]*R2D,azel[1]*R2D,v[nv],y,sqrt(sig),obs[i].SNR[f]*0.25,f);
                     ...
                     int nx, int stat)
{
    ...
    /* current filter time */
    if (stat==1) {
        flt->time=sol->time=timeadd(obs_time,-x[AIC(0,0,opt)]/CLIGHT);
    }
    else if (lsq->psta) {
    ...
    for (i=0;i<3;i++) {
        sol->rr[i]=x[i];
        sol->qr[i]=flt->P[i+i*nx];
    }
    sol->qr[3]=flt->P[1+0*nx];
    sol->qr[4]=flt->P[2+1*nx];
    sol->qr[5]=flt->P[2+0*nx];

    /* receiver clock/clock drift error */
    for (i=0;i<4;i++) {
        for (f=0;f<NFREQ;f++) sol->dtr[AIC_(i,f)]=flt->x[AIC(i,f,opt)];
    }
    sol->ddrt=flt->x[AID(opt)];
    ...
    trace(3,"updfltsol: rr=%10.4lf %10.4lf %10.4lf vel=%10.4lf %10.4lf %10.4lf\n",
          ...
          flt->x[3],flt->x[4],flt->x[5],vn[0],vn[1],vn[2]);
...
static int valsol(int type, agpos_t *agp, const double *v, const int *ind, int nv, const double *var,
                  ...
                  const aobsd_t *obs, double *vr, int *clk_jump_ind)
{
    double *g,*R,*M,*N;
    double thres_gain=4.0,thres_res=4.0;
    int i,flag=1,pos_jump=0,vel_jump=0;
    unsigned int iflg[6]={0};

    trace(3,"valsol: nv=%d\n",nv);

    g=mat(nx,1); R=mat(nv,nv);
    N=mat(nv,nv); M=mat(nv,nv);
    ...
    for (*clk_jump_ind=-1,i=0;i<nx;i++) {
        if ((vr[i]=fabs(g[i]/sqrt(Pp[i+i*nx])))>thres_gain) {
            ...
            else if (i<3) pos_jump++;
            else if (i<6) vel_jump++;
        }
        else {
            vr[i]=0.0;
        }
    ...
    for (i=0;i<nv;i++) {
        ...
        if (vr[nx+i]>thres_res) {
            trace(2,"large post-fit residual: sat=%3d f=%d type=%d cn0=%6.3lf r=%.3lf\n",
                  obs[AOBS_I(ind[i])].sat,AOBS_F(ind[i]),AOBS_T(ind[i]),
                       ...
                       const aobsd_t *obs, int *exc)
{
    ...
    for (j=-1,i=0;i<nv;i++) {
        ...
        if (j<0||fabs(vr[i])>fabs(vr[j])) j=i;
    }
    if (j>=0) {
        exc[AEXC_I(AOBS_I(ind[j]),AOBS_F(ind[j]))]=1;
        trace(2,"exclude: sat=%3d f=%d type=%d\n",
              obs[AOBS_I(ind[j])].sat,AOBS_F(ind[j]),AOBS_T(ind[j]));
    }
    return j;
}
/* filter model validation---------------------------------------------------*/
static int val_fltmdl(int type, const double *v, const int *ind, const double *H,
                      const double *var, const aobsd_t *obs, double *x, double *P, int nv, int nx, int *exc)
{
    double *Rv,*vv,thres=re_norm(0.999),s;
    int i,ne=0;
    ...
    for (i=0;i<nv;i++) {
        ...
        if (exc[AEXC_I(AOBS_I(ind[i]),AOBS_F(ind[i]))]) continue;
        exc[AEXC_I(AOBS_I(ind[i]),AOBS_F(ind[i]))]=1;
        ne++;

        trace(2,"exclude: sat=%3d f=%d type=%d\n",obs[AOBS_I(ind[i])].sat,
              AOBS_F(ind[i]),AOBS_T(ind[i]));
    }
    free(Rv); free(vv);
    return nv-ne;
}
...
/* reset rcv clock-----------------------------------------------------------*/
static void reset_clka(double *xp, double *Pp, int nx, const double clk_est, int i, int f,
                       const prcopt_t *opt)
{
    trace(3,"reset_clk: i=%d\n",i);

    initxP(xp,Pp,xp[AIC(i,f,opt)]+clk_est,VAR_CLK,AIC(i,f,opt),nx);
                           ...
                           const prcopt_t *opt)
{
    ...
    initxP(xp,Pp,xp[AID(opt)]+clk_dri_est,VAR_CLK,AID(opt),nx);
...
static int detoutl_meas(const double *v, const int *ind, int nv, int *exc, const aobsd_t *obs,
                        double thres_box, double thres_mad, double thres_grb)
{
    ...
    for (i=0;i<n;i++) {
        if (exc[AEXC_I(AOBS_I(id[outl[i]]),AOBS_F(id[outl[i]]))]) continue;
        exc[AEXC_I(AOBS_I(id[outl[i]]),AOBS_F(id[outl[i]]))]=1;

        trace(2,"exclude: sat=%3d f=%d type=%d CN0=%6.3lf\n",
              obs[AOBS_I(id[outl[i]])].sat,AOBS_F(id[outl[i]]),AOBS_T(id[outl[i]]),
                           ...
                           const nav_t *nav, agpos_t *agp, double *xp, double *Pp)
{
    ...
    const double min_cn0=MAX(7.0,agp->spp.lsq.min_cn0[0]),min_el=agp->opt.elmincode;
    double *v,*vi,med,clk_est;
    int i,j,f,m,mk,nx=agp->spp.flt.nx,nv=n*NFREQ*n+3,*ind,*flag;
    int *outl,noutl,jump_flg=0;

    trace(3,"detect_clk_jump: n=%d\n",n);

    v=mat(nv,1); vi=mat(nv,1);
    ind=imat(nv,1); outl=imat(nv,1); flag=imat(nv,1);
    ...
    /* i=0:gps/qzs,1:glo,2:gal,3:bds */
    for (i=0;i<4;i++) {
        for (f=0;f<NFREQ;f++) {
            for (m=j=0;j<nv;j++) {
                if (obs[AOBS_I(ind[j])].SNR[AOBS_F(ind[j])]*0.25<min_cn0) continue;
                if (AOBS_F(ind[j])!=f) continue;
                if (ind_sys(satsys(obs[AOBS_I(ind[j])].sat,NULL))!=i) continue;
                if (agp->ssat[obs[AOBS_I(ind[j])].sat-1].azel[1]<min_el) continue;
                vi[m++]=v[j];
            }
            ...
            if (mk<2) continue;
            med=median(vi,mk);
            if (fabs(mad(vi,mk)/med)>thres) continue;
            if (fabs(stdv(vi,mk)/avg(vi,mk))>thres) continue;

            /* detect rcv clock jump */
            if (fabs(med)/sqrt(Pp[AIC(i,f,opt)+AIC(i,f,opt)*agp->spp.flt.nx])>thres) {
                reset_clka(xp,Pp,nx,med,i,f,&agp->opt);
                jump_flg++;
            }
        }
    }
                                 ...
                                 const nav_t *nav, agpos_t *agp, double *xp, double *Pp)
{
    ...
    trace(3,"detect_clk_drift_jump: n=%d\n",n);
    ...
    if (m<3||mad(vm,m)>1.0) return 0;
    med=median(vm,m);

    /* detect rcv clock drift jump */
    if (fabs(med)/sqrt(Pp[AID(opt)+AID(opt)*agp->spp.flt.nx])>thres) {
...
static int updfilt_prc(int type, meas_res_func_t *res_func, const aobsd_t *obs, int n,
                       const nav_t *nav, agpos_t *agp, double *xp, double *Pp)
{
    double *x,*P,*v,*H,*var,*xt,*Pt,*R,*vr,clk_est;
    int clk_jump_ind,reset_cnt=-1,uLSQ=1;
    int i,j,nx=agp->spp.flt.nx,nv=2*NFREQ*n+3,stat=0,*ind,*exc,info;
    ...
    Pt=mat(nx,nx); P=mat(nx,nx); R=mat(nv,nv);
    x=mat(1,nx); H=mat(nv,nx); v=mat(1,nv); vr=mat(nx+nv,1);
...
sppfltrestart:
...
#if DETECT_OUTL
    /* measurement residuals before exclude outliers */
    if ((nv=res_func(obs,n,nav,-1,1,uLSQ,1,x,rtk,v,H,var,exc,ind))<=0) {
        trace(2,"no valid meas\n");
        stat=-1;
        goto sppfltquit;
    }
    ...
    /* filter model validation */
    if ((stat=val_fltmdl(type,v,ind,H,var,obs,x,P,nv,nx,exc))<=0) {
    ...
    for (stat=i=0;i<MAXITR;i++) {
...
#else
        /* do ekf filter */
        if (filter(x,P,H,v,R,nx,nv,NULL,0)) {
            trace(2,"ekf filter error\n");
            stat=-2;
            break;
        }
        ...
        if ((info=valsol(type,agp,v,ind,nv,var,H,xt,Pt,x,P,nx,obs,vr,&clk_jump_ind))==1) {
            matcpy(xp,x, 1,nx);
            matcpy(Pp,P,nx,nx);
            stat=1;
            break;
...
sppfltquit:
    ...
    free(exc); free(ind); free(var); free(R);
...
#define TIME_INTTERP  1
    ...
    if (!solp||!solr) {
        ...
        if (!solr) trace(2,"no rtk solution\n");
        return;
    }
    ...
    fprintf(fp_dbg,"%10.4lf  "
            ...
            lsq->sig0_p1,lsq->sig0_v1,gdp_h,gdv_h,
...
extern int asppflt(agpos_t *agp, const aobsd_t *obs, int n, const nav_t *nav)
{
    ...
    trace(3,"asppflt: sow=%6.3lf n=%d\n",time2gpst(obs[0].time,NULL),n);

    Pp=zeros(nx,nx); xp=zeros(1,nx);

    /* initialize EKF estimated states */
    if (!init_flt_state(agp,obs,n,1,0,NULL,NULL)) {
        trace(2,"EKF filter initial fail\n");
        free(xp); free(Pp);
        return 0;
    }
    /* time update of EKF filter */
    if (!timeupdflt(agp,obs,n,nav,xp,Pp)) {
        trace(2,"EKF filter update fail\n");
        free(xp); free(Pp);
        return 0;
    }
    /* update GNSS measurement data */
    stat=updmeasflt(obs,n,agp,nav,opt,xp,Pp);

    /* update EKF filter solution */
