...
const double chisqr[150]={      /* chi-sqr(n) (alpha=0.001) */
    ...
    150 ,151 ,153 ,154 ,155 ,156 ,157 ,159 ,160 ,161 ,
    162 ,163 ,165 ,166 ,167 ,168 ,170 ,171 ,172 ,173 ,
    174 ,176 ,177 ,178 ,179 ,180 ,181 ,183 ,184 ,185 ,
    186 ,187 ,189 ,190 ,191 ,192 ,193 ,195 ,196 ,197 ,
    198 ,199 ,201 ,202 ,203 ,204 ,205 ,205 ,206 ,208
};
const prcopt_t prcopt_default={ /* defaults processing options */
    PMODE_KINEMA,1,SYS_GPS|SYS_GLO|SYS_GAL|SYS_CMP|SYS_QZS,  /* mode,ar,navsys */
    {{0}},                      /* SNR mask */
    15.0*D2R,                   /* elevation mask angle (rad) for pseudorange */
    15.0*D2R,                   /* elevation mask angle (rad) for doppler */
    15.0*D2R,                   /* elevation mask angle (rad) for RTK */
    EPHOPT_BRDC,                /* satellite ephemeris/clock (EPHOPT_???) */
    1,5,                        /* minlock,minfix */
    IONOOPT_BRDC,TROPOPT_SAAS,  /* ionosphere option,troposphere option */
    1,30.0,3.0,0.3,0.5,0.5,     /* dynamics,maxgdop,thresar{ratio,Ps,ADOP,CDOP} */
    15.0,15.0,0.03,30.0,        /* elmaskar,elmaskinherit,thresslip,maxtdiff */
    {3.0,3.0,0.1,5.0,0.1,0.01,0.001},
                                /* GNSS position process PSD */
    15.0,15.0,30.0,             /* min_cn0_code, min_cn0_dopp, min_cn0_phas */
    {"",""},                    /* anttype */
    {{0}},{{0}},                /* antdel,pcv */
    0,                          /* fix GLONASS ambiguity */
    {0.0},                      /* base station position (ECEF/m) */
    1,                          /* velocity filter (0:off,1:on) */
    0,                          /* PPP option (0:off,1:on) */
    1,                          /* update RTK solution by adjust sigma (available for GNSS process) */
    1,                          /* update STD of DD residuals */
    0,                          /* GNSS process direction (0:forward, 1:backward) */
    5,                          /* max iteration for RTK filter */
    1,                          /* inherit DD ambiguity options (0:off,1:on) */
    {{0.0},{0.0}},              /* ocean tide loading parameters {rov,base} */
    5e-10,                      /* satellite clock stability (sec/sec) */
    1
};
...
static char codepris[7][MAXFREQ][16]={  /* code priority for each freq-index */
    ...
    {"C"       ,"IQX"       ,""        ,""       ,""       ,""      ,""}, /* SBS */
    {"IQXDPAN" ,"IQXDPZ"    ,"IQDPX"   ,"IQXA"   ,"DPX"    ,""      ,""}, /* BDS */
    {"ABCX"    ,"ABCX"      ,""        ,""       ,""       ,""      ,""}  /* IRN */
};
                   ...
                   const double *A, const double *B, double beta, double *C)
{
    ...
    for (i=0;i<n;i++) for (j=0;j<k;j++) {
            d=0.0;
            switch (f) {
                case 1: for (x=0;x<m;x++) d+=A[i+x*n]==0.0||B[x+j*m]==0.0?0.0:A[i+x*n]*B[x+j*m]; break;
                case 2: for (x=0;x<m;x++) d+=A[i+x*n]==0.0||B[j+x*k]==0.0?0.0:A[i+x*n]*B[j+x*k]; break;
                case 3: for (x=0;x<m;x++) d+=A[x+i*m]==0.0||B[x+j*m]==0.0?0.0:A[x+i*m]*B[x+j*m]; break;
                case 4: for (x=0;x<m;x++) d+=A[x+i*m]==0.0||B[j+x*k]==0.0?0.0:A[x+i*m]*B[j+x*k]; break;
            }
...
static void hhbg(int n,double *a)
{
    ...
    for (k=1;k<=n-2;k++) {
        ...
        if (fabs(d)+1.0!=1.0) {
            if (i!=k) {
                for (j=k-1;j<=n-1;j++) {
                    u=i*n+j; v=k*n+j; t=a[u]; a[u]=a[v]; a[v]=t;
                }
                for (j=0;j<=n-1;j++) {
                    u=j*n+i; v=j*n+k; t=a[u]; a[u]=a[v]; a[v]=t;
                }
            }
            for (i=k+1;i<=n-1;i++) {
...
static int qrtt(int n,double *a,double *u,double *v,double eps,int jt)
{
    ...
    while (m!=0) {
        ...
        if (l==m-1) {
            u[m-1]=a[(m-1)*n+m-1]; v[m-1]=0.0; m=m-1; it=0;
        }
        else if (l==m-2) {
            b=-(a[ii]+a[ll]); c=a[ii]*a[ll]-a[jj]*a[kk]; w=b*b-4.0*c; y=sqrt(fabs(w));
            if (w>0.0) {
                xy=1.0;
                if (b<0.0) xy=-1.0;
                u[m-1]=(-b-xy*y)/2.0; u[m-2]=c/u[m-1]; v[m-1]=0.0; v[m-2]=0.0;
            }
            else {
            ...
            for (k=l;k<=m-2;k++) {
                if (k!=l) {
                    ...
                    x=a[ii]+a[ll]; y=a[ll]*a[ii]-a[kk]*a[jj];
                    ii=l*n+l; jj=l*n+l+1; kk=(l+1)*n+l; ll=(l+1)*n+l+1;
                    p=a[ii]*(a[ii]-x)+a[jj]*a[kk]+y; q=a[kk]*(a[ii]+a[ll]-x); r=a[kk]*a[(l+2)*n+l+1];
                }
                if ((fabs(p)+fabs(q)+fabs(r))!=0.0) {
                    ...
                    if (k!=l) a[k*n+k-1]=-s;
                    e=-q/s; f=-r/s; x=-p/s; y=-x-f*r/(p+s); g=e*r/(p+s); z=-x-e*q/(p+s);

                    for (j=k;j<=m-1;j++) {
                        ii=k*n+j; jj=(k+1)*n+j; p=x*a[ii]+e*a[jj]; q=e*a[ii]+y*a[jj]; r=f*a[ii]+g*a[jj];
                        if (k!=m-2) {
                            kk=(k+2)*n+j; p=p+f*a[kk]; q=q+g*a[kk]; r=r+z*a[kk]; a[kk]=r;
                        }
                        a[jj]=q; a[ii]=p;
                    }
                    j=k+3;
                    if (j>=m-1) j=m-1;
                    for (i=l;i<=j;i++) {
                        ii=i*n+k; jj=i*n+k+1; p=x*a[ii]+e*a[jj]; q=e*a[ii]+y*a[jj]; r=f*a[ii]+g*a[jj];
                        if (k!=m-2) {
                            kk=i*n+k+2; p=p+f*a[kk]; q=q+g*a[kk];r=r+z*a[kk]; a[kk]=r;
                        }
                        a[jj]=q; a[ii]=p;
                    }
                }
...
{
    double *A_=mat(n,n),EPS=1E-8;
    int i=0,ITERS=100;

    matcpy(A_,A,n,n);
    hhbg(n,A_);
    i=qrtt(n,A_,u,v,EPS,ITERS); free(A_);
...
{
    double *Rm=mat(nv,nv),*T;

    matcpy(Rm,R,nv,nv);
    if (matinv(Rm,nv)) {
        free(Rm); return 0;
    }
    matmul33("NNT",H,Rm,H,nx,nv,nv,nx,Q);

    if (matinv(Q,nx)) {
        free(Rm); return 0;
    }
    T=mat(nv,1);
    matmul33("NNN",H,Rm,v,nx,nv,nv,1,T);
    matmul("NN",nx,1,nx,1.0,Q,T,0.0,dx);
    free(Rm); free(T); return 1;
}
/* kalman filter ---------------------------------------------------------------
...
static int filter_(const double *x, const double *P, const double *H,
                   ...
                   double *xp, double *Pp, double *Ko)
{
    double *F=mat(n,m),*Q=mat(m,m),*K=mat(n,m),*I=eye(n);
    double *P1=mat(n,n),*P2=mat(n,n);
    int i,j,info;
...
#if 1
        matmul33("NNT",I,P,I,n,n,n,n,P1);
        matmul33("NNT",K,R,K,n,m,m,n,P2);
        for (i=0;i<n;i++) {
            for (j=0;j<n;j++) Pp[i+j*n]=P1[i+j*n]+P2[i+j*n];
        }
#else
        matmul("NN",n,n,n,1.0,I,P,0.0,Pp);
#endif
        if (Ko) matcpy(Ko,K,n,m);
    }
                  ...
                  const double *R, int n, int m, const int *ix_, int nx_)
{
    ...
    info=filter_(x_,P_,H_,v,R,k,m,xp_,Pp_,NULL);
    for (i=0;i<k;i++) {
        x[ix[i]]=xp_[i];
...
extern int adrbfilter(double *x, double *P, const double *H, const double *v,
                      const double *R, int n, int m, const int *ix_, int nx_)
{
    double *x_,*xp_,*P_,*Pp_,*H_,*dx,*S,*F,vs,tr,a,b,maxr;
    double *R_,*P1,*P2,*K,*s,*r,*I,*y;
    double c0=2.5,c1=6.5,c3=0.1,k0=2.5,k1=6.5;
    int i,j,k,l,info=0,*ix,maxiter=15;
    ...
    x_=mat(k,1); xp_=mat(k,1); R_=mat(m,m);
    P_=mat(k,k); Pp_=mat(k,k); P1=mat(k,k); P2=mat(k,k);
    H_=mat(k,m); dx=mat(k,1); S=mat(m,m); F=mat(k,m); K=mat(k,m);
    r=mat(1,m); I=eye(k); y=mat(m,m); s=mat(m,1);
    ...
    for (vs=i=0;i<m;i++) vs+=v[i]*v[i];
    for (i=0;i<maxiter;i++) {
        matcpy(S,R_,m,m);
        matmul("NN",k,m,k,1.0,P_,H_,0.0,F);
        matmul("TN",m,m,k,1.0,H_,F,1.0,S);

        tr=mattrace(S,m);
        a=fabs(vs/tr);

        if      (a>c1) b=1E10;
        else if (a>c0) b=(c0/a)*SQR((c1-a)/(c1-c0));
        else           b=1.0;

        matcpy(S,R_,m,m);
        matmul("NN",k,m,k,1.0,P_,H_,0.0,F);
        matmul("TN",m,m,k,1.0/b,H_,F,1.0,S);

        if (matinv(S,m)) {
            info=-1;
            break;
        }
        matmul("NN",k,m,k,1.0/b,P_,H_,0.0,F);
        matmul("NN",k,m,m,1.0,F,S,0.0,K);

        matmul("NN",k,1,m,1.0,K,v,0.0,dx);
        matmul("NT",k,k,m,-1.0,K,H_,1.0,I);

        matmul33("NNT",I,P_,I,k,k,k,k,P1);
        matmul33("NNT",K,R_,K,k,m,m,k,P2);

        for (j=0;j<k;j++) xp_[j]=x_[j]+dx[j];
        for (j=0;j<k;j++) for (l=0;l<k;l++) Pp_[l+j*k]=P1[l+j*k]+P2[l+j*k];

        matmul("TN",m,1,k,1.0,H_,dx,0.0,r);
        for (j=0;j<m;j++) r[j]=v[j]-r[j];

        matmul33("TNN",H_,Pp_,H_,m,k,k,m,y);
        for (j=0;j<m;j++) s[j]=fabs(r[j]/SQRT(fabs(R_[j+j*m]-y[j+j*m])));

        for (maxr=-1E10,l=-1,j=0;j<m;j++) {
            if (l<0||maxr<s[j]) l=j,maxr=s[j];
        }
        if (l>=0) {
            if      (s[l]>k1) R_[l+l*m]*=1E10;
            else if (s[l]>k0) R_[l+l*m]/=(k0/s[l])*SQR((k1-s[l])/(k1-k0));
            else break;
        }
    }
    for (i=0;i<k;i++) {
    ...
    free(ix); free(x_); free(xp_); free(P_); free(Pp_); free(H_);
    free(R_); free(P1); free(P2); free(S); free(dx); free(F); free(K);
    free(r); free(y); free(I); free(s);
    return info;
}
/* iterate EKF----------------------------------------------------------------*/
...
{
    double *x_,*xp_,*P_,*Pp_,*H_,*dx,*K,*T,*dg;
    int i,j,k,info,*ix;

    ix=imat(n,1);
    if (ix_) {
        for (i=k=0;i<nx;i++) ix[k++]=ix_[i];
    }
    else {
        for (i=k=0;i<n;i++) if (x[i]!=0.0&&P[i+i*n]>0.0) ix[k++]=i;
    }
    x_=mat(k,1); xp_=mat(k,1);
    P_=mat(k,k); Pp_=mat(k,k);
    H_=mat(k,m); dx=mat(1,k); K=mat(n,m);
    T=mat(n,n); dg=mat(1,n);
    ...
    info=filter_(x_,P_,H_,v,R,k,m,xp_,Pp_,K);

    for (i=0;i<k;i++) dx[i]=x[ix[i]]-xp[ix[i]];
    matmul("NT",k,k,m,1.0,K,H_,1.0,T);
    matmul("NN",k,1,k,1.0,T,dx,1.0,dg);
    ...
    for (i=0;i<k;i++) dx[i]=x[ix[i]]-xp[ix[i]];
    if (norm(dx,k)<1E-2) info=1;

    free(ix); free(x_); free(xp_); free(P_); free(Pp_); free(H_);
    free(dx); free(dg); free(K); free(T);
    return info;
}
/* smoother --------------------------------------------------------------------
...
{
    return (uint64_t)((t.time+t.sec)*1E9);
}
...
{
    struct sta *psta=sta,*csta[MAXRCV];
    int i,c=0;

    while (psta) {
        if (!(psta=psta->next_sta)) break;
        csta[c++]=psta;
    }
    for (i=0;i<c;i++) {
        free(csta[i]);
...
extern void traceobs(int level, const obsd_t *obs, int n)
{
    ...
    for (i=0;i<n;i++) {
        ...
        trace(3," (%2d) %s(%15.8lf) %-3s rcv%d %13.3f %13.3f %13.3f %13.3f %13.3f %13.3f %13.3f %13.3f %13.3f %2d %2d %2d %2d %2d %2d %5.1f %5.1f %5.1f\n",
              i+1,str,time2gpst(obs[i].time,NULL),id,obs[i].rcv,obs[i].L[0],obs[i].L[1],obs[i].L[2],obs[i].P[0],
              obs[i].P[1],obs[i].P[2],obs[i].D[0],obs[i].D[1],obs[i].D[2],obs[i].LLI[0],obs[i].LLI[1],obs[i].LLI[2],obs[i].code[0],
              obs[i].code[1],obs[i].code[2],obs[i].SNR[0]*SNR_UNIT,obs[i].SNR[1]*SNR_UNIT,obs[i].SNR[2]*SNR_UNIT);
    }
    fflush(fp_trace);
}
...
extern double geodist(const double *rs, const double *rr, double *e)
{
    ...
    if (norm(rs,3)<RE_WGS84) return 0.0;
...
extern double norm_distri(double u)
{
    ...
    double p=1.0+y*(0.0705230784+y*(0.0422820123+y*(0.0092705272+y*(0.0001520143+
...
extern double re_norm(double p)
{
    ...
    y=y*(1.57079628800000+y*(0.37069879060E-1+y*(-0.8364353589E-3+y*(-0.2250947176E-3+y*(0.68412182990E-05+
      y*(0.58242385150E-5+y*(-0.1045274970E-5+y*(0.83609370170E-7+y*(-0.3231081277E-8+y*(0.36577630360E-10+
      y*0.6936233982E-12))))))))));
    return sqrt(y);
}
/* B distribution functional---------------------------------------------------*/
extern double B(int n1, int n2, double x, double  *Ux)
{
    int m1,m2;
    double Ix,pi=3.14159265358979312;
...
/* t distribution functional---------------------------------------------------*/
extern double t(int nn, double tv, double *f)
{
    if(tv+1.0==1.0)return 0.5;
    double x=tv*tv/(nn+tv*tv),P=0.5*B(1,nn,x,f);
...
{
    if(p<0.5) return -re_t(n,1.0-p);
    double x,pi=3.14159265358979312;

    if(n==1) x=tan(pi*(p-0.5));
    else if(n==2) {
        double u=2.0*p-1.0;
        u=u*u; x=sqrt(2.0*u/(1.0-u));
    }
    else {
        x=re_norm(p)*sqrt(n/(n-2.0));
        while(1) {
            double f,F=t(n,x,&f),dx=(F-p)/f;
            x=x-dx;
...
/* F distribution functional---------------------------------------------------*/
extern double F(int n1, int n2, double x, double *f)
{
    double y=n1*x/(n2+n1*x),Ux,p=B(n1,n2,y,&Ux);
    *f=Ux/x;
    return p;
}
...
{
    double d,f0,a=2.0/(9.0*n1),b=2.0/(9.0*n2);
    double a1=1.0-a,a2,b1=1.0-b;
    double u=re_norm(p),e=b1*b1-b*u*u;

    if (e<=0.8) {
        a1=2.0*pow(n2+0.0,0.5*n2-1.0);
        a2=pow(n1+0.0,0.5*n2);
        double B=gam(n1)*gam(n2)/gam(n1+n2),f=a1/a2/B/(1-p);
        f0=pow(f,2.0/n2);
    ...
    if (f0<0.0) f0=0.01;
    while(1) {
        double pp=F(n1,n2,f0,&d),df=(p-pp)/d;
        while (fabs(f0)<fabs(df)) df/=2.0;
        f0=f0+df;
        if( fabs(df)/f0<0.0001||fabs(df)<0.0001) return f0;
    }
}
static double mnorm1(const double *A, int m, int n)
{
    double s=0.0,smax=0.0;
    int i,j;
    for (i=0;i<m;i++) {
        for (s=j=0;j<n;j++) s+=fabs(A[i+j*n]);
        if (s>smax) smax=s;
    }
    return smax;
}
...
{
    double thetam=0.0001,s,n1,ss,c;
    double *B=eye(n),*C=zeros(n,n),*Nm=eye(n),*Dm=eye(n);
    int i,j,k,p=13,q=13;

    matcpy(Z,A,n,n);

    n1=mnorm1(A,n,n);
    s=ROUND(MAX(8,log(n1/thetam)/log(2)));
    ss=pow(2,s);

    for (i=0;i<n;i++) {
        for (j=0;j<n;j++) Z[i+j*n]/=ss;
    }
    for (i=0;i<=p;i++) {
        c=factorial(p+q-i)*factorial(p)/(factorial(p+q)*factorial(i)*factorial(p-i));
        matmul("NN",n,n,n,c,B,Z,0.0,C);
        matcpy(B,C,n,n);
        for (j=0;j<n;j++) for (k=0;k<n;k++) Nm[j+k*n]+=C[j+k*n];
    }
    for (i=0;i<=q;i++) {
        c=factorial(p+q-i)*factorial(q)/(factorial(p+q)*factorial(i)*factorial(q-i));
        matmul("NN",n,n,n,-c,B,Z,0.0,C);
        matcpy(B,C,n,n);
        for (j=0;j<n;j++) for (k=0;k<n;k++) Dm[j+k*n]+=C[j+k*n];
    }
    if (!matinv(Dm,n)) {
        matmul("NN",n,n,n,1.0,Dm,Nm,0.0,B);
        for (i=0;i<s;i++) {
            matmul("NN",n,n,n,1.0,B,B,0.0,Z);
            matcpy(B,Z,n,n);
        }
    }
    free(B); free(C); free(Nm); free(Dm);
}